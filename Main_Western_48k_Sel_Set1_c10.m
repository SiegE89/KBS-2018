...
Data1  = ['[Western][48k][Xdat2][feat6][Rank][c10].mat'];
% Carga de la base de datos
...
% Nombre del archivo que contiene la base de datos
Data3  = ['[Western][Xdat2][feat6][BoW][c10].mat'];
% Carga de la base de datos
...
% save('[Western][FeatSel][Set1][c10][CheckPoint]')
...
%     for  z = 1 : Nw %loop feature selection approaches
    for  z = 3
        for j = 1 : length(vv) % loop features
    ...
    tc(i) =  toc;
    fprintf('e.time=%.4f[s]\n',tc(i));
%     save('[Western][FeatSel][Set1][c10]','Mir','Wr','acc_knn','par_opt','indices','Nw','Cmknn','vv');
end